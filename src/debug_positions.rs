...
pub fn debug_satellite_positions(
    ...
    println!(
        "Время: GPS Week {}, MS {}",
        cur_time.Week, cur_time.MilliSeconds
    );
    let transmit_time_full =
        (cur_time.Week as f64) * 604800.0 + (cur_time.MilliSeconds as f64) / 1000.0;
    let transmit_time_week = (cur_time.MilliSeconds as f64) / 1000.0;
    println!(
        "transmit_time полное: {:.1} сек, секунды недели: {:.1}",
        transmit_time_full, transmit_time_week
    );
    println!(
        "Позиция приёмника: ({:.1}, {:.1}, {:.1})",
        receiver_pos.x, receiver_pos.y, receiver_pos.z
    );

    let receiver_lla = ecef_to_lla(receiver_pos);
    println!(
        "Приёмник LLA: lat={:.6}°, lon={:.6}°, alt={:.1}m",
    ...
    for svid in &c_version_gps {
        ...
        if let Some(eph) = &gps_eph[idx] {
            ...
            if gps_sat_pos_speed_eph(
                GnssSystem::GpsSystem,
                transmit_time_week_secs,
                &mut eph_mut,
                &mut sat_pos,
                None,
            ) {
                ...
                let range = ((sat_pos.x - receiver_pos.x).powi(2)
                    ...
                    + (sat_pos.z - receiver_pos.z).powi(2))
                .sqrt();
                ...
                println!(
                    "{} | El={:5.1}° Az={:5.1}° Range={:.0}km | toe={} health={}",
                    status,
