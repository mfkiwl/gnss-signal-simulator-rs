...
impl FNavBit {
    ...
    pub fn set_almanac(&mut self, alm: &[GpsAlmanac]) -> i32 {
        ...
        for i in 0..12 {
            ...
            if start_idx + 2 < alm.len() {
                FNavBit::compose_alm_words(
                    &alm[start_idx..start_idx + 3],
    ...
    fn compose_eph_words(ephemeris: &GpsEphemeris, eph_data: &mut [[u32; 7]; 4]) -> i32 {
        ...
        eph_data[0][5] =
            COMPOSE_BITS!(int_value, 11, 10) | COMPOSE_BITS!((ephemeris.health >> 2) & 0x1, 10, 1);
    ...
    fn compose_alm_words(almanac: &[GpsAlmanac], alm_data: &mut [[u32; 7]; 2], week: i32) -> i32 {
        ...
        // PageType 5
        alm_data[0][0] = COMPOSE_BITS!(5, 16, 6)
            | COMPOSE_BITS!(4, 12, 4)
            | COMPOSE_BITS!(week, 10, 2)
        ...
        if almanac.len() > 1 {
            ...
            let int_value =
                Self::unscale_int((almanac[1].i0 - NOMINAL_I0) / std::f64::consts::PI, -14);
            ...
            if almanac.len() > 2 {
                ...
                let int_value =
                    Self::unscale_int((almanac[2].i0 - NOMINAL_I0) / std::f64::consts::PI, -14);
                ...
                alm_data[1][6] |=
                    COMPOSE_BITS!(if (almanac[2].valid & 1) != 0 { 0 } else { 1 }, 3, 2);
            }
        }
