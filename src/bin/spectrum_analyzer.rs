...
fn read_iq_data(filename: &str, format: &str, max_samples: usize) -> Result<Vec<Complex>, Box<dyn std::error::Error>> {
...
fn plot_spectrum_ascii(frequencies: &[f64], psd: &[f64], center_freq: f64, width: usize) {
    ...
    println!("{:<.1} кГц          {:.1} кГц          {:.1} кГц", freq_min, freq_center, freq_max);
    ...
    for (idx, &(freq, power)) in peaks.iter().take(10).enumerate() {
        ...
        println!("{:2} │ {:13.3} │ {:+11.1} │ {:13.1} │ {:8.1}", 
                idx + 1, freq / 1e3, doppler, power, snr);
    }
...
fn save_spectrum_csv(filename: &str, frequencies: &[f64], psd: &[f64]) -> Result<(), Box<dyn std::error::Error>> {
    use std::io::Write;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Использование: {} <if_data_file> [format] [sample_rate] [center_freq]", args[0]);
    ...
    let sample_rate: f64 = args.get(3)
        .and_then(|s| s.parse().ok())
        .unwrap_or(5_000_000.0);
    let center_freq: f64 = args.get(4)
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.0);
