    ...
    #[cfg(feature = "gpu")]
    {
        if let Ok(cuda_accelerator) = CudaGnssAccelerator::new() {
            ...
            for _ in 0..100 {
                ...
                let _ = cuda_accelerator.complex_signal_processing_gpu(
                    &dummy_data,
                    &dummy_data,
                    &dummy_data,
                    &dummy_data,
...
pub fn benchmark_satellite_processing(
    num_satellites: usize,
    samples_per_sat: usize,
        ...
        for sat_id in 0..num_satellites {
            ...
            for chunk in (0..samples_per_sat).step_by(16) {
                ...
                for i in 0..chunk_size {
                    let phase =
                        ((chunk + i) as f64 * sat_id as f64 * 0.001) % (2.0 * std::f64::consts::PI);
